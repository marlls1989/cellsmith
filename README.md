...
| Liberty stub | `<name>.lib` | a self-contained `library (<name>) { ... }` file (Liberate can consume it directly) wrapping one `cell (...)` per cell: input `pin`s; a sequential cell gets one joint `statetable` listing every state node — state outputs and genuine internals alike, each under its own name — and each output pin is classified against it: an output that **is** a state node carries a `state_function` naming that node, an output that **depends on** state nodes carries a `state_function` over them, and an output over primary inputs alone carries a plain `function`. Genuine internal nodes get `direction : internal` pins carrying their `internal_node`. A cell with no state nodes gets a plain `function` per output and no `statetable` |
