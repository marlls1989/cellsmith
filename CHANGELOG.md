...
- **The hazard report states one entry per cause, and each outcome names its own nodes.** The header names the timing and the state it goes wrong from; the body names the condition, the walk into that state, and then one field per outcome observed, each listing the victim nodes that reading names. The header carries no node set, because two outcomes of one cause need not attack the same nodes — an SR latch's set pulse rings over `{Q, Qn}` and settles indeterminately over `{Q, Qn, L}`.
