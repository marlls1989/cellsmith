...
- **Detection reports every hazard it observes, and emission decides what actually renders.** Detection deduplicates, ranks and selects nothing: a hazard observed from ten reachable states is ten records, under either cause and either outcome. A canonical hazard used to be kept only from the state reached along the shortest prevector, with every other state it was observed from discarded before emission saw them. Emission now groups a constraint's observations by everything that identifies it except the nodes it protects, orders those node sets by containment, and gives a general block — no `-when`, standing for the constraint however it was reached — to each set no other in the group contains. Being contained demotes an observation rather than dropping it: it renders no general block and still renders its own conditioned block, over its own nodes, in the input context it was observed in, so a conditioned block can probe less than any general one. That containment rule now applies to every constraint kind, where it used to reach the minimum pulse widths alone.

- **A constraint is one situation's — its cause, its input condition, and the nodes it protects.** All three components count: observations agreeing on all three are readings of one probe and collapse onto one constraint, while observations differing in any one of them are separate claims and each is stated. The same pins racing with a side input held high put a different question to the characterisation than with it held low, and the setup/hold and non_seq separations now tell those apart as the minimum pulse widths already did — each rendering its own conditioned block under `--when`.
