...
- **cellsmith detects a pulse too short to be honoured, and constrains its minimum width.** A pulse on one input is its two own edges: toggle it out of a state the cell has settled in, let the cascade run, toggle it back. Where the cell lands depends on how far the cascade got between the two edges, and a pulse whose landing disagrees with the one a fully-settled cell reaches — or which leaves the cell ringing — is a hazard the width causes. Each one yields a single-pin minimum-pulse-width constraint, emitted as a `-type min_pulse_width` Liberate block stating one edge on the pulsed pin, every other input held, and the nodes whose settled value the width decides on `-probe`. The vector states one edge, so Liberate performs the pulse and searches the width itself; nothing changes in the `.lib`, since Liberate writes the characterised `min_pulse_width_high`/`_low` into its own output library.

- **cellsmith classifies a hazard on two independent axes — cause and outcome — and detects all four combinations.** A hazard's cause is what its timing is between: a race between two inputs (or a single input observed racing alone), or a pulse on one input's own two edges. Its outcome is what the machine then does: settling indeterminately, or oscillating. A hazard kind is a value on an axis rather than a record type carrying its own detection pass, dedup key and emitter branch, which is what lets the pulse above reach the deck through the same records, the same collapse and the same emitter a race does. cellsmith generates a constraint from every hazard whose cause states a timing to hold — a directed setup/hold or symmetric non-sequential separation for a race, a minimum pulse width for a pulse.

- **`--when=constraint` characterises a constraint in each input context it was observed in.** The `-when`-conditioned pass covered the delay/transition and hidden arcs; it now covers the derived constraint arcs too, so `--when=constraint` — or `when = "constraint"` on a cell — adds one conditioned block per observation beside the general block that stands for the constraint however it was reached.

### Changed

- **Detection reports every hazard it observes, and emission decides what actually renders.** Detection deduplicates, ranks and selects nothing: a hazard observed from ten reachable states is ten records, under either cause and either outcome. A canonical hazard used to be kept only from the state reached along the shortest prevector, with every other state it was observed from discarded before emission saw them. Emission now groups a constraint's observations by everything that identifies it except its victim nodes — the state variables a hazard attacks — orders those node sets by containment, and gives a general block — no `-when`, standing for the constraint however it was reached — to each set no other in the group contains. Being contained demotes an observation rather than dropping it: it renders no general block and still renders its own conditioned block, over its own nodes, in the input context it was observed in, so a conditioned block can probe less than any general one. That containment rule now applies to every constraint kind, where it used to reach the minimum pulse widths alone.

- **A constraint is keyed on its cause: the pins with the edge each makes, and the state the probe acted from.** Observations agreeing on that are readings of one cause and collapse onto one constraint, whose `-probe` is the union of the victim nodes each of them named — one block covering every consequence the cause has, since the timing that removes the cause removes them all. The nodes at risk are the effect and so no part of the key; the starting state is, because a state-holding cell reaches one input assignment in several stored states — a C-element holds either value under `A*!B` — and keying on the input condition folded those together before emission could see them. They now reach emission separately, where a block whose `-ic` and `-vector` cannot tell them apart is named by the masked-arc warning, and what to do about it is the spec author's call.

- **`when` is the input assignment a transition happens FROM, on every arc of every kind.** A block's `-when` states the standing assignment its measured transition starts at, and the pins it switches are written as edges rather than as literals of the condition. Race-cause hazards used to carry the assignment the machine is left in once both edges have landed, which disagreed with the `-when` the block beside them wrote: a mutex's ring is now reported and annotated under `!A*!B`, the idle state its two requests rise out of, rather than under the `A*B` they land in.

- **The hazard report states one entry per cause, and each outcome names its own nodes.** The header names the timing and the state it goes wrong from; the body names the condition, the walk into that state, and then one field per outcome observed, each listing the victim nodes that reading names and where the machine lands on them when the timing is honoured — for a race the alternatives it may settle to, for a pulse the rest states an adequately wide one walks through. The header carries no node set, because two outcomes of one cause need not attack the same nodes — an SR latch's set pulse rings over `{Q, Qn}` and settles indeterminately over `{Q, Qn, L}`.

- **A lone toggle that never settles is detected however few inputs a cell has.** A race between two pins needs two pins, and the pair probes still early out below that; the single-toggle probes no longer share that early-out, so a one-input ring — `Q = "!(EN*Q)"` — is reported instead of passing silently.

- **`--constraints` takes no pin name.** It is a blanket flag: it asks every cell for constraint arcs on every input pin. Which pins one cell wants is the spec's own `constraint_arcs`, which takes `true`, a pin name or a list, and rejects a name that is not one of that cell's declared inputs.

- **Naming a pin selects the constraints that pin has a role in, and the roles are the kind's.** A non-sequential separation is symmetric — its two pins are equals — so naming either end selects the separation that holds them apart. A setup/hold is directed, the data pin being constrained with respect to the clock, so its data pin selects it: naming the clock asks for what that clock is itself subject to, its own minimum pulse width, and not for the separations other pins are held around it by. A minimum pulse width is selected by the pin it pulses. The answer is read off the constraint's kind rather than off whichever end is stored as its constrained pin, so it no longer follows the order the cell happens to declare its inputs in.

- **An oscillation is annotated on the constraint it motivated.** The `# oscillation:` comment in the emitted Tcl, and the `/* oscillation: … */` form in the `.lib`, lead the constraint block generated from the ringing observation instead of heading the file. A comment explains what it accompanies, so a ring with no constraint beside it carries none — a ring observed under a lone toggle names one pin, and one edge has nothing to be separated from. Every detected hazard reaches the user through the report on stderr either way.
