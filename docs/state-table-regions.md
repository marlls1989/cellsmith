...
— outputs then internals. The Verilog and Liberty emitters read that cache rather than rebuilding the
BDDs (§7).
...
relay/alias internal (like `sela` here) has no surviving output, so it has no statetable and no region
entry at all.
...
The emitters read these regions rather than rebuilding them: `on` becomes `1`/`H`, `off` becomes `0`/`L`,
and `hold` becomes `-`/`N` in the Verilog sequential UDP and the Liberty `statetable` respectively, with
internal state nodes appearing as internal pins and as internal-node columns in the tables of the
outputs that reference them.
